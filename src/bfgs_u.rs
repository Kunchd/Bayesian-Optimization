// The following code is taken from https://github.com/paulkernfeld/bfgs?tab=readme-ov-file
// the crate was not used due to version differences of dependent crates.
...
fn line_search<F, B>(f: F, in_bound: B) -> Result<f64, ()>
where
    F: Fn(f64) -> f64,
    B: Fn(f64) -> bool
{
...
/// - `bounds` is an array of \[lower, upper\] bounds for each element in x0
/// - `f` is the objective function
/// - `g` is the gradient of `f`
#[allow(clippy::many_single_char_names)]
pub fn bfgs<F, G>(x0: Array1<f64>, bounds: &Vec<f64>, f: F, g: G) -> Result<Array1<f64>, ()>
where
    F: Fn(&Array1<f64>) -> f64,
    G: Fn(&Array1<f64>) -> Array1<f64>,
{
    ...
    loop {
        ...
        // function defined to check if given epsilon is in bound
        let in_bound = |epsilon: f64| -> bool {
            let step = &search_dir * epsilon + &x;
            // println!("step being considered: {:?}", step);
            for &elt in step.iter() {
                // println!("elt: {}, under bounds: {}, {}", elt, bounds[i][0], bounds[i][1]);
                if bounds[0] > elt || bounds[1] < elt {
                    return false
                }
            }
            true
        ...
        // println!("x for starting line search: {:?}", x);
        let epsilon = line_search(|epsilon| f(&(&search_dir * epsilon + &x)), in_bound).map_err(|_| ())?;
